 ...
 * 壁挂式道路指示牌方块实体渲染器。
 *
 * <p>1.21.11 起 BlockEntityRenderer 改为 render-state 模式：先在
 * {@link WallRoadSignBlockEntityRenderer#updateRenderState} 把方块实体数据写入
 * {@link WallRoadSignBlockEntityRenderState}，再在
 * {@link WallRoadSignBlockEntityRenderer#render} 提交渲染指令到
...
public class WallRoadSignBlockEntityRenderer<T extends WallSignBlockEntity>
        implements net.minecraft.client.render.block.entity.BlockEntityRenderer<T, WallRoadSignBlockEntityRenderState> {
    ...
    @Override
    public WallRoadSignBlockEntityRenderState createRenderState() {
        return new WallRoadSignBlockEntityRenderState();
    }

    @Override
    public void updateRenderState(T entity, WallRoadSignBlockEntityRenderState state, float tickProgress, Vec3d cameraPos, @Nullable net.minecraft.client.render.command.ModelCommandRenderer.CrumblingOverlayCommand crumblingOverlay) {
        ...
        state.zOffset = getZOffsetForWallRoadSign(state.facing);
    }
     ...
     */
    @Override
    public void render(WallRoadSignBlockEntityRenderState state, MatrixStack matrices, OrderedRenderCommandQueue queue, CameraRenderState cameraState) {
        // 发光时使用最大光照
        ...
        matrices.translate(0, 0, state.zOffset);
