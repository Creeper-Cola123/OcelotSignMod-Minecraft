...
public class ModelSelectionOverlay extends Overlay {
    ...
    /**
     * 由 {@link bklmc.ocelotsign.mixin.client.MouseScrollMixin} 调用，传入 GLFW 的原始滚轮值。
     *
    ...
    public void handleMouseScrolledFromMixin(double horizontal, double vertical) {
        ...
        // Minecraft 的 onMouseScroll 中:
        // - vertical > 0 表示滚轮向上滚动（wheel toward user）
        // - vertical < 0 表示滚轮向下滚动
        // 向上滚动应该使列表向下移动（scrollY 减小，露出下面的内容）
        // 所以应该是 scrollY = scrollY - vertical * amount = scrollY + |vertical| * amount
