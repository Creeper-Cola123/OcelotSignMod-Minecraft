...
public class ModelSelectionOverlay extends Overlay {
    ...
    private void renderHeader(DrawContext ctx) {
        ...
        ctx.drawStrokedRectangle(PANEL_MARGIN, headerBottom - 1, panelWidth - PANEL_MARGIN * 2, 1, UIConstants.COLOR_MAIN_BORDER);
    ...
    private void renderFooter(DrawContext ctx, int mx, int my) {
        ...
        ctx.fill(backBtnX, footerTop,
                ...
                btnBg);
        ctx.drawStrokedRectangle(backBtnX, footerTop,
                UIConstants.RETURN_BUTTON_WIDTH, UIConstants.RETURN_BUTTON_HEIGHT,
    ...
    private void drawEmptyIcon(DrawContext ctx, int cx, int cy) {
        ...
        ctx.drawStrokedRectangle(cx - size, cy + 4, size, size, UIConstants.COLOR_ITEM_BORDER);
        ctx.fill(cx - size + 7, cy - 3, cx + 7, cy + size + 3, UIConstants.COLOR_ITEM_BG);
        ctx.drawStrokedRectangle(cx - size + 7, cy - 3, size, size, UIConstants.COLOR_ITEM_BORDER);
        ctx.fill(cx - size + 14, cy - 10, cx + 14, cy + size - 10, UIConstants.COLOR_ITEM_BG);
        ctx.drawStrokedRectangle(cx - size + 14, cy - 10, size, size, UIConstants.COLOR_ITEM_BORDER);
    ...
    private void renderScrollbar(DrawContext ctx, int mx, int my) {
        ...
        ctx.drawStrokedRectangle(sx, thumbY, UIConstants.SCROLLBAR_WIDTH, thumbH, UIConstants.COLOR_ITEM_BORDER);
    }
    ...
    public void tickInput() {
        ...
        double mx = mc.mouse.getScaledX(mc.getWindow());
        double my = mc.mouse.getScaledY(mc.getWindow());
        // 直接通过 GLFW 轮询鼠标左键按下状态，不依赖 Mouse.wasLeftButtonClicked()
        // （后者在 Overlay 模式下行为不可靠）。
        boolean mouseDown = org.lwjgl.glfw.GLFW.glfwGetMouseButton(glfwHandle, GLFW.GLFW_MOUSE_BUTTON_LEFT) == GLFW.GLFW_PRESS;
        ...
        // ESC 键关闭
        if (org.lwjgl.glfw.GLFW.glfwGetKey(glfwHandle, GLFW.GLFW_KEY_ESCAPE) == GLFW.GLFW_PRESS) {
            closeAndPlaceBlock();
        }
    }
    ...
    public void handleMouseScrolledFromMixin(double horizontal, double vertical) {
        ...
        if (maxScrollY > 0 && vertical != 0) {
            scrollY = MathHelper.clamp(scrollY - vertical * SCROLL_AMOUNT, 0, maxScrollY);
        }
    }
    ...
    class ModelEntry {
        ...
        public void render(DrawContext ctx, int x, int y, int rowWidth, int mx, int my, int panelLeft, int panelRight) {
            ...
            if (rowHover) {
                ctx.drawStrokedRectangle(panelLeft, y, panelRight - panelLeft, ROW_HEIGHT, UIConstants.COLOR_ITEM_BORDER);
            }
            ...
            ctx.drawStrokedRectangle(btnX, btnY, BTN_W, BTN_H, UIConstants.COLOR_BTN_BORDER);
