 ...
 * 模型选择界面（无3D旋转效果版本）
 *
 * <p>当资源包未加载时，使用描边（Outline）渲染效果代替3D旋转预览。
 *
...
public class ModelSelectionScreen extends Screen {
    ...
    private void renderBottomRightHints(DrawContext ctx) {
        ...
        int bx = width - maxW - padding * 2 - margin;
        int by = height - lineH * 2 - padding * 2 - margin;

        ctx.fill(bx, by, width - margin, height - margin, 0xAA000000);
     ...
     * 未加载资源包时，使用描边（Outline）渲染效果代替3D旋转预览。
     */
    class ModelEntry {
        ...
        public void render(DrawContext ctx, int x, int y, int w, int h, int mx, int my) {
            ...
            // 渲染模型预览（描边效果代替3D旋转）
            renderModelOutline(ctx, innerL, innerT, innerR, innerB);
         ...
         * 渲染模型描边效果（代替3D旋转预览）。
         *
         * <p>当资源包未加载时，使用简化的描边矩形来表示模型预览区域。
         * 这与 D:\MCDevelopment\ocelotsignmod-template-1.21.11 版本保持一致。
         *
         * @param ctx 绘制上下文
         * @param left 预览区左边界
         * @param top 预览区上边界
         ...
         * @param bottom 预览区下边界
         */
        private void renderModelOutline(DrawContext ctx, int left, int top, int right, int bottom) {
            int centerX = (left + right) / 2;
            int centerY = (top + bottom) / 2;
            int boxSize = Math.min(right - left, bottom - top) - 8;

            // 计算盒子位置（居中）
            int boxLeft = centerX - boxSize / 2;
            int boxTop = centerY - boxSize / 2;
            int boxRight = boxLeft + boxSize;
            int boxBottom = boxTop + boxSize;

            // 外层描边矩形
            ctx.drawBorder(boxLeft, boxTop, boxSize, boxSize, UIConstants.COLOR_ITEM_BORDER);

            // 内层描边矩形（偏移4像素）
            int innerOffset = 4;
            ctx.drawBorder(boxLeft + innerOffset, boxTop + innerOffset,
                    boxSize - innerOffset * 2, boxSize - innerOffset * 2, UIConstants.COLOR_ITEM_BORDER);

            // 最内层描边矩形（再偏移4像素）
            int inner2Offset = 8;
            ctx.drawBorder(boxLeft + inner2Offset, boxTop + inner2Offset,
                    boxSize - inner2Offset * 2, boxSize - inner2Offset * 2, UIConstants.COLOR_ITEM_BORDER);

            // 在最内层绘制 "?" 问号表示模型占位符
            Text placeholder = Text.literal("?");
            int placeholderW = textRenderer.getWidth(placeholder);
            ctx.drawText(textRenderer, placeholder,
                    centerX - placeholderW / 2,
