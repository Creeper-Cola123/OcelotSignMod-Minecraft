 ...
 * 和 {@link ScreenMouseEvents} 在 {@link bklmc.ocelotsign.OcelotSignModClient}
 * 中统一轮询派发。
 */
public class ModelSelectionScreen extends Screen {
    ...
    public boolean mouseScrolled(double mx, double my, double horizontalAmount, double verticalAmount) {
        if (mx >= panelWidth) return super.mouseScrolled(mx, my, horizontalAmount, verticalAmount);

        int contentTop = PANEL_MARGIN + headerHeight + CONTENT_TOP_PAD;
        int contentBottom = cachedHeight - PANEL_MARGIN - footerHeight - 10;

        if (my < contentTop || my > contentBottom) return super.mouseScrolled(mx, my, horizontalAmount, verticalAmount);
