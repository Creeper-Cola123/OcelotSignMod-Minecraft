...
public final class ColorPickerRenderer {
    ...
    private static void renderSvPanel(DrawContext context, int svX, int svY, int svSize) {
        ...
        if (ColorPickerState.getSvTextureId() != null) {
            context.drawTexture(RenderPipelines.GUI_TEXTURED, ColorPickerState.getSvTextureId(),
        ...
        context.drawStrokedRectangle(svX, svY, svSize, svSize, 0xFF222222);
        ...
        context.drawStrokedRectangle(ix - indicatorPad, iy - 1, indicatorPad * 2, 2, 0xFF222222);
        context.drawStrokedRectangle(ix - 1, iy - indicatorPad, 2, indicatorPad * 2, 0xFF222222);
    }
    ...
    private static void renderHueBar(DrawContext context, int hueX, int hueY, int hueW, int svSize) {
        ...
        context.drawStrokedRectangle(hueX, hueY, hueW, svSize, 0xFF222222);
        ...
        context.drawStrokedRectangle(hueX - hueIndicatorPad, hueIndicatorY - 1, hueW + hueIndicatorPad * 2, 2, 0xFF222222);
    }
    ...
    private static void renderColorPreview(DrawContext context, TextRenderer textRenderer, int previewX, int previewY, int previewW) {
        ...
        context.drawStrokedRectangle(previewX, previewY, previewW, previewH, 0xFF222222);
        context.drawStrokedRectangle(previewX - 1, previewY - 1, previewW + 2, previewH + 2, 0xFF888888);
    ...
    private static void renderPresetColors(DrawContext context, TextRenderer textRenderer, int previewX, int previewY, int previewW) {
        ...
        for (int i = 0; i < ColorPickerState.COLOR_PICKER_PRESETS.length; i++) {
            ...
            context.drawStrokedRectangle(sx, sy, presetSize, presetSize, 0xFF555555);
            if ((rgb & 0xFFFFFF) == (currentRgb & 0xFFFFFF)) {
                context.drawStrokedRectangle(sx - 1, sy - 1, presetSize + 2, presetSize + 2, 0xFFFFE088);
            }
        }
    }
    ...
    private static void renderCopyButton(DrawContext context, TextRenderer textRenderer, int btnX, int btnY, int btnW, int btnH) {
        ...
        context.drawStrokedRectangle(btnX, btnY, btnW, btnH, UIConstants.COLOR_BTN_BORDER);
